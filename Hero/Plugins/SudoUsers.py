...
async def useradd(client, message: Message, _):
    ...
    if not message.reply_to_message:
        ...
        if user.id in SUDOERS:
            return await message.reply_text(
                _["dhskbdnd"].format(user.mention)
            )
        added = await add_sudo(user.id)
        if added:
            SUDOERS.add(user.id)
            await message.reply_text(_["dhksbd"].format(user.mention))
        else:
...
async def userdel(client, message: Message, _):
    ...
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text(_["bdksndjs"])
        user = message.text.split(None, 1)[1]
        if "@" in user:
            user = user.replace("@", "")
        user = await app.get_users(user)
        if user.id not in SUDOERS:
            return await message.reply_text(_["dhksndd"])
        removed = await remove_sudo(user.id)
        if removed:
            SUDOERS.remove(user.id)
            await message.reply_text(_["bdmsndnsns"])
            return
        await message.reply_text(f"Something wrong happened.")
        return
    user_id = message.reply_to_message.from_user.id
    if user_id not in SUDOERS:
        return await message.reply_text(_["dbjsndnd"])
    removed = await remove_sudo(user_id)
    if removed:
        SUDOERS.remove(user_id)
        await message.reply_text(_["bdmsndnsn"])
        return
    await message.reply_text(f"Something wrong happened.")
