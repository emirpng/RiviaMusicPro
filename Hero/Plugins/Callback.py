...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "resumecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Şarkı {CallbackQuery.from_user.mention} tarafından devam ettirildi.",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "stopcb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Şarkı {CallbackQuery.from_user.mention} tarafından sonlandırıldı. Asistan sohbetten ayrılıyor.",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "skipcb":
        ...
        if Queues.is_empty(chat_id):
            ...
            await remove_active_video_chat(chat_id)
            await CallbackQuery.message.reply_text(
                f"Sırada daha fazla müzik yok \n\nAsistan sesli sohbetten ayrıldı. Butonu kullanan :- {CallbackQuery.from_user.mention}"
            )
            await stop_stream(chat_id)
            await CallbackQuery.message.delete()
            await CallbackQuery.answer(
            ...
            if str(finxx) == "s1s":
                ...
                mystic = await CallbackQuery.message.reply_text(
                    "Atlandı!  Lütfen Video Akışını Değiştirmeyi Bekleyin...."
                )
...
async def play_playlist(_, CallbackQuery):
    ...
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Canlı Akış Oynatılıyor... Çalma listesini oynatmak için durdurun",
                show_alert=True,
            )
        else:
            pass
    ...
    if not _playlist:
        ...
        await CallbackQuery.message.delete()
        mystic = await CallbackQuery.message.reply_text(
            f"Oynatma Listesini Başlatma {third_name}.\n\Tarafından:- {CallbackQuery.from_user.first_name}"
        )
        msg = f"Sıraya Alınmış Playlist:\n\n"
        j = 0
        for_t = 0
        for_p = 0
