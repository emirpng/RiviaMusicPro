...
@app.on_message(
    commandpro(["ytp", "/ytp", "/ytoynat", "/ytplay", "/ytp@{BOT_USERNAME}"]) & filters.group
)
@checker
@logging
@AssistantAdd
async def play(_, message: Message):
    ...
    if audio:
        mystic = await message.reply_text(
            "🔄 Ses işleniyor... Lütfen Bekle!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
                return await mystic.edit(
                    "Canlı Akış Oynatılıyor...Müzik oynatmak için durdur."
                )
            else:
                pass
        except:
        ...
        if (audio.duration) > DURATION_LIMIT:
            return await mystic.edit_text(
                f"**Süre limiti aşıldı.**\n\n**İzin verilen: **{DURATION_LIMIT_MIN} minute(s)\n**Oynatmak İstediğiniz:** {duration_min} minute(s)"
            )
        file_name = (
        ...
        if not limit:
            return await message.reply_text(
                "**Görüntülü aramalar için sınır tanımlanmadı**\n\nBotta izin verilen maksimum görüntülü arama sayısı için bir sınır belirleyin: `/set_video_limit` [Yalnızca SudoUsers]"
            )
        ...
        if int(count) == int(limit):
            if await is_active_video_chat(message.chat.id):
                ...
                return await message.reply_text(
                    "Bot, CPU aşırı yükleme sorunları nedeniyle yalnızca sınırlı sayıda görüntülü görüşmeye izin verir.  Diğer birçok sohbet şu anda görüntülü aramayı kullanıyor.  Sese geçmeyi deneyin veya daha sonra tekrar deneyin."
                )
        mystic = await message.reply_text(
...
async def Music_Stream(_, CallbackQuery):
    ...
    if str(duration) == "None":
        ...
        return await CallbackQuery.edit_message_text(
            "**Canlı akış algılandı.**\n\nOynatmak isterseniz mevcut oynatılan şarkı durdurulacak ve akışa başlanacak...",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
