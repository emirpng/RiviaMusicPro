...
async def mplay_stream(message,MusicData):
    ...
    try:
        ...
        if read1:
            return await message.reply_text("Canlı yayın oynatılıyor./nMüzik oynatmak için durdur.")
        else:
            pass
    except:
        pass
    ...
    mystic = await message.reply_text(f"🔄 Müzik İndiriliyor... Lütfen Bekleyin... {title[:20]}")
    await mystic.edit(
        f"**🔄 Müzik İndiriliyor... Lütfen Bekleyin... {title[:50]}"
    )
...
async def custom_video_stream(
    ...
    if await is_active_chat(message.chat.id):
        ...
        cap = f"**Video Yayını**\n\n🎥<b>Oynatılan: </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n💡Bilgi:</b>\n👤**İsteyen:** {message.from_user.mention}"
        final_output = await message.reply_photo(
            photo=thumb,
            reply_markup=InlineKeyboardMarkup(buttons),
            caption=cap,
